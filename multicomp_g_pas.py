...
def runSimulation(sim,class_sample,theta_sample,run_c_param,args):
    ...
    for i in range(class_sample):
        ...
        for cl in sim.classes:
            ...
            tmp.append(drawFromInterval(1e-05,0.0008,class_sample,i))
...
#    class_param_prob=map(lambda x:[x[0],fsum(map(lambda y:exp(-y/(2*args["noise_params"][1]**2)),x[1]))/theta_sample],class_param_prob)
    ...
    fig5.savefig("prob_distro_g_pas.svg", dpi=None, facecolor='w', edgecolor='w')
...
def main():
    ...
    sim.class_params.append("g_pas")
    sim.theta_params.append("Ra")
    sim.theta_params.append("cm")
    sim.ff.usr_fun(sim.ff,[0.0003,151,7.5])         
...
#    plt.title("autocorrelation")
    ...
    #plt.show()
    fig1.savefig("base_trace_g_pas.svg", dpi=None, facecolor='w', edgecolor='w')
