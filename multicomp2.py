...
class simulationEnv(object):
    def __init__(self):
        ...
        self.theta_distr = [(150,30),(0.0005,0.0001)]
...
#    class_param_prob=map(lambda x:[x[0],fsum(map(lambda y:exp(-y/(2*args["noise_params"][1]**2)),x[1]))/theta_sample],class_param_prob)
    ...
    fig5.savefig("prob_distro_cm.svg", dpi=None, facecolor='w', edgecolor='w')
...
#    plt.title("autocorrelation")
    ...
    runSimulation(sim, 200, 30, run_c_param, {"noise_params": [noise_mean,noise_dev]})
    #plt.show()
    fig1.savefig("base_trace_cm.svg", dpi=None, facecolor='w', edgecolor='w')
